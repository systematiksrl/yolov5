...
def save_image(image, id, directory = 'images' , ext = '.png'):
    ...
    image_path = directory + f'img' + id + ext
    
    counter = 0

    while os.path.exists(image_path):
        image_path = directory+f'{div}img'+id+'_'+str(counter)+ext
        counter += 1
...
def run(
        ...
        iou_thres=0.45,  # stdout save cropped prediction boxes
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            #print('image shape: %g x %g  ' % im.shape[2:])  # print string
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if vid_path[i] != save_path:  # new video
                        ...
                        else:  # stream                            fps, w, h = 30, im0.shape[1], im0.shape[0]                        save_path = str(Path(save_path).with_suffix('.mp4'))  # force *.mp4 suffix on results videos
